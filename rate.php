...
if (1 == $xoopsModuleConfig['loggedinvote']) {
    #echo "logged in ";
...
if (isset($_REQUEST['id']) and isset($_REQUEST['rate'])) {
    ...
    $result = $xoopsDB->query('SELECT id FROM ' . $xoopsDB->prefix('amreview_rate') . " WHERE rate_user_ip LIKE '%" . $_SERVER['REMOTE_ADDR'] . "%' AND rate_review_id = '" . (int)$_GET['id'] . "'");
    ...
    if ($voted) { // match
        ...
        redirect_header('review.php?id=' . (int)$_GET['id'] . '', 2, _AM_AMREV_ALRDYVTD);
    ...
    if ($xoopsDB->getAffectedRows($sql)) {
        redirect_header('review.php?id=' . (int)$_GET['id'] . '', 2, _AM_AMREV_VOTED);

    //echo "voted";
    } else {
        redirect_header('review.php?id=' . (int)$_GET['id'] . '', 2, _AM_AMREV_DBVOTEFAIL);
...
if (isset($_REQUEST['somethingsoIdonotgetloaded'])) {
    $GLOBALS['xoopsOption']['template_main'] = 'amr_rate.html';

 ...
 * @return string|string[]
 */
function getCatPath($topcap, $catid, $columnname, $path, $delim, $table, $itemID, $parID)
{
    global $xoopsDB;
    ...
    $mytree = new XoopsTree($xoopsDB->prefix((string)$table), (string)$itemID, (string)$parID);
...
function amr_getReviewcount($catid = '0')
{
    ...
    $sql = ('SELECT COUNT(id) AS count FROM ' . $xoopsDB->prefix('amreview_reviews') . " WHERE catid='" . $catid . "'");
...
function amr_increment_views($id)
{
    ...
    $sql = ('UPDATE ' . $xoopsDB->prefix('amreview_reviews') . " SET views=views+1 WHERE id='" . (int)$id . "'");
...
function amr_getGetsubcats($catid = '0')
{
    ...
    $sql = ('SELECT * FROM ' . $xoopsDB->prefix('amreview_cat') . " WHERE cat_parentid='" . $catid . "' ORDER BY cat_title ASC");
...
function getUserRating($id = '0')
{
    ...
    $result = $xoopsDB->query('SELECT AVG(rate_rating) AS rate, COUNT(rate_rating) AS votes FROM ' . $xoopsDB->prefix('amreview_rate') . " WHERE rate_review_id='" . (int)$id . "'");

    [$rate, $votes] = $xoopsDB->fetchRow($result); // {

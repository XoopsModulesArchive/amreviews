...
if (isset($_REQUEST['id'])) {
    $GLOBALS['xoopsOption']['template_main'] = 'amr_review.html';
    ...
    if ($xoopsDB->getRowsNum($result) > 0) {
        while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
            ...
            #$voted=@$GLOBALS['xoopsDB']->fetchArray(@$GLOBALS['xoopsDB']->queryF("SELECT title FROM $tableName WHERE used_ips LIKE '%".$_SERVER['REMOTE_ADDR']."%' AND id='$id' "));
            ...
            #$userRatings = $ratingsHandler->getRating(intval($_GET['id']));
            ...
            $userRatings = getUserRating((int)$_GET['id']);
            ...
            if ($page >= $pageno) {
                ...
                $nextlink = '<a href="review.php?id=' . (int)$_GET['id'] . '&amp;page=' . $next_pagenumber . '">' . _MD_AMR_PAGENEXT . '</a>';
            ...
            if ($page <= 1) {
                ...
                $prevlink = '<a href="review.php?id=' . (int)$_GET['id'] . '&amp;page=' . $previous_pagenumber . '">' . _MD_AMR_PAGEPREV . '</a>';
            ...
            if ($page <= 1) {
                if (!$xoopsUser or ($xoopsUser->isAdmin($xoopsModule->mid()) and 1 != $xoopsModuleConfig['noincrementifadmin'])) {

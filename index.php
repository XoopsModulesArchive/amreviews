...
if (!isset($_REQUEST['op'])) {
    $GLOBALS['xoopsOption']['template_main'] = 'amr_index.html';
    ...
    $sql = ('SELECT * FROM ' . $xoopsDB->prefix('amreview_cat') . ' ' . $sqlquery . '');
    ...
    if ($xoopsDB->getRowsNum($result) > 0) {
        while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
            ...
            //$amr_cat['rev_count'] = amr_getReviewcount($myrow['id']);
            ...
            if (1 == $xoopsModuleConfig['showsubcats']) {
                $amr_cat['subcatsswitch'] = 1;

                $amr_cat['subcats'] = amr_getGetsubcats($myrow['id']);
            }
    ...
    $cat_path = getCatPath($xoopsModule->getVar('name'), $catid, 'cat_title', 'index.php?t=', '&#187;', 'amreview_cat', 'id', 'cat_parentid');
    ...
    if (isset($_GET['id'])) {
        ...
        // intval($_GET['category_id']) - $xoopsModule->getVar('mid')

        if ($gpermHandler->checkRight('Category Permission', (int)$_GET['id'], $groups, $xoopsModule->getVar('mid'))) {
            // allowed, so display contents within the category
            //echo "hello";
        } else {
            // not allowed, display an error message or redirect to another page

            redirect_header(XOOPS_URL . '/user.php', 3, _MD_AMR_NOPERMCATMSG);
        ...
        $sql = ('SELECT * FROM ' . $xoopsDB->prefix('amreview_reviews') . " WHERE (date_publish='0' OR " . time() . " > date_publish) AND (date_end='0' OR " . time() . " < date_end) AND validated='1' AND showme='1' AND catid='" . (int)$_GET['id'] . "' ORDER BY date DESC");

        //" WHERE (date_publish > 0 AND date_publish <= '". time() ."') AND (date_end = 0 OR date_end > '". time() ."')"); //AND id='" . intval($_GET['id']) . "'");
        ...
        if ($xoopsDB->getRowsNum($result) > 0) {
            while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
                ...
                if (1 == $xoopsModuleConfig['showreviewedby']) {
                    // later add realname option - https://www.xoops.org/misc/api/kernel/XoopsUser.html#getUnameFromId
            ...
            $xoopsTpl->assign('noreviewscap', _MD_AMR_NOREVIEWCAP);
        }
    } // end if
